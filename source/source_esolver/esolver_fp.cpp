...
void ESolver_FP::before_all_runners(BaseCell& basecell, const Input_para& inp)
{
    ...
    const bool kin_den = this->chr.kin_density(inp.out_elf[0] > 0); // mohan add 20251202
	this->chr.allocate(inp.nspin, kin_den, inp.test_charge); // mohan move this from setup_estate_pw, 20251128
