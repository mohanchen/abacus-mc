    ...
    ModuleBase::realArray &table_local)
{
    ...
    for(int it=0; it<ucell.ntype; it++)
    {
        ...
        for(int L=0; L<ucell.atoms[it].nwl+1; L++)
        {
            for(int N=0; N<ucell.atoms[it].l_nchi[L]; N++)
            {
                ...
                ModuleBase::GlobalFunc::ZEROS(radial.data(), meshr);
                ModuleBase::GlobalFunc::ZEROS(psi.data(), meshr);
                ModuleBase::GlobalFunc::ZEROS(psir.data(), meshr);
                ModuleBase::GlobalFunc::ZEROS(rab.data(), meshr);
                ...
                std::vector<double> table(PARAM.globalv.nqx);
                Wavefunc_in_pw::integral(
                    ucell, meshr, psir.data(), radial.data(), rab.data(), L, table.data());
    ...
    if(GlobalV::MY_RANK==0)
    {
        ...
        {
            std::stringstream ss;
            ss << PARAM.globalv.global_out_dir << ucell.atoms[it].label << "/local_g.txt";
                                               ...
                                               const ModuleBase::realArray& table_local)
{
    ...
    std::vector<double> chiaux;
    bool chiaux_init = false;
                            ...
                            if(ucell.atoms[it].ncpp.has_so)
                            {
                                ...
                                {//atomic_wfc_so_mag
                                    ...
                                    std::complex<double> fup,fdown;
                                      //int nc;
                                      // This routine creates two functions only
                                       // in the case j=l+1/2 or exit otherwise
                                    ...
                                    } else
                                    {
                                        ...
                                        {
                                            if(
                                                (ucell.atoms[it].lchi[ib] == L) &&
                                                (fabs(ucell.atoms[it].jjj[ib] - L + 0.5) < 1e-4)
                                            )
                                            {
                                            nc=ib;
                                            break;
                                    ...
                                    for(int m = 0;m<2*L+1;m++)
                                    {
                                        ...
                                        for(int ig = 0;ig<npw;ig++)
                                        {
                                            ...
                                            psi(iwall, ig) =
                                                (cos(0.5 * gamma) + ModuleBase::IMAG_UNIT * sin(0.5 * gamma))
                                                * fup;
                            ...
                            {//atomic_wfc_nc
                                ...
                                for(int m = 0;m<2*L+1;m++)
                                {
                                    ...
                                    for(int ig = 0;ig<npw;ig++)
                                    {
                                        ...
                                        //first rotation with angle(alpha+ModuleBase::PI) around(OX)
                                        psi(iwall, ig) =
                                            (cos(0.5 * gamman) + ModuleBase::IMAG_UNIT * sin(0.5 * gamman))
                                            * fup;
                                        psi(iwall, ig + wfc_basis->npwk_max)
