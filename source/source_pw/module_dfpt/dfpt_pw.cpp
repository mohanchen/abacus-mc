...
void DFPT_PW::Impl::build_occ_kq(int q_idx)
{
    ...
    for (int ik = 0; ik < nk; ++ik)
    {
        ...
        const int dn_i[3] = {static_cast<int>(dnr[0]), static_cast<int>(dnr[1]), static_cast<int>(dnr[2])};
        ...
        for (int m = 0; m < nbands; ++m)
        {
            ...
            for (int igl = 0; igl < npw_kq; ++igl)
            {
                ...
                const std::vector<int> key = {static_cast<int>(std::round(gf.x)) + dn_i[0],
                ...
                if (pimpl_->wired())
                {
                    ...
                    for (int iat = 0; iat < nat; ++iat)
                    {
                        for (int idir = 0; idir < 3; ++idir)
                        {
                            ...
                            pimpl_->phon_
                                .accumulate_electron(q_idx, iat, idir, pimpl_->gs_psi_, pimpl_->wg_, pimpl_->data_);
                        }
                    }
                    pimpl_->data_.add_residual(q_idx, irrep, worst);
                    pimpl_->data_.set_converged(q_idx, irrep, worst < pimpl_->data_.get_conv_thr());
                }
                else
                {
