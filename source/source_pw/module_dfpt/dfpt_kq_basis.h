...
class DFPT_KQ_Basis
{
  public:
    ...
    int get_ig_rho(int igl) const
    {
        return ig_rho_[igl];
    }
    ///< G in Cartesian coordinates
    ModuleBase::Vector3<double> get_gcar(int igl) const
    {
        return gcar_[igl];
    }
    ///< G + (k+q) in Cartesian coordinates
    ModuleBase::Vector3<double> get_gpluskq(int igl) const
    {
        return gcar_[igl] + kplusq_c_;
    }
    ///< |G + (k+q)|^2 in units of 1/lat0^2
    double get_gk2(int igl) const
    {
        return gk2_[igl];
    }
    ///< k+q wavevector in Cartesian coordinates
    ModuleBase::Vector3<double> get_kplusq() const
    {
        return kplusq_c_;
    }
    const std::vector<double>& get_gk2_all() const
    {
        return gk2_;
    }
    const std::vector<ModuleBase::Vector3<double>>& get_gcar_all() const
    {
        return gcar_;
    }
