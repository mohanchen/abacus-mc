...
void Bessel_Basis::init_Faln(const int& ntype, const int& lmax, const int& nmax, const int& ecut_number, const UnitCell& ucell)
{
                                 ...
                                 const double& tolerence)
{
    ...
    const int output_rmesh = cutoff_is_on_grid ? cutoff_rmesh + (cutoff_rmesh % 2 == 0 ? 1 : 0) : rmesh;
    ...
    for (int l = 0; l < lmax + 1; l++)
    {
        ...
        for (int ie = 0; ie < ecut_number; ie++)
        {
            ...
            // caoyu add 2021-3-10
            //=========output .orb format=============
            ofs << std::setiosflags(std::ios::right) << std::setw(20) << "Type" << std::setw(20) << "L" << std::setw(20) << "N"
                << std::endl;
            ofs << std::setiosflags(std::ios::right) << std::setw(20) << "0" << std::setw(20) << l << std::setw(20) << ie << std::endl;
            for (int ir = 0; ir < output_rmesh; ir++)
            {
                // Taoni fix 2026-07-23: Output zero for points beyond the cutoff radius
...
void Bessel_Basis::allocate_C4(const int& ntype, const int& lmax, const int& nmax, const int& ecut_number, const UnitCell& ucell)
{
